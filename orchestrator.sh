...
main() {
  ...
  configure_cloud_init \
    "$VMID" "$VM_NAME" "$VM_STORAGE" "$SNIPPETS_STORAGE" "$CI_USER" "$ANSIBLE_AUTH_MODE" "$ANSIBLE_SSH_KEY_PATH" "$ANSIBLE_SSH_KEY_TEXT" "$ANSIBLE_PASSWORD" "$ROOT_AUTH_MODE" "$ROOT_SSH_KEY_PATH" "$ROOT_SSH_KEY_TEXT" "$ROOT_PASSWORD" "$IP_MODE" "$IP_CIDR" "$GATEWAY" "$DNS_SERVER"
